...
**同一设计，两个已交付的产品。** [Majalis](https://github.com/Nas01010101/majalis)——我们的
Track 3 智能体社会——其共享信念板采用与 Tenet 完全相同的取代设计（同样的
`entity::attribute` 键碰撞语义、退役而非删除，作为独立的进程内实现）：四个异构 Qwen
智能体实时写入，一个学习到的世界模型读取信念状态来决定辩论是否值得开销。该机制不止
适用于个人记忆——它在第二个产品中承担着核心负载。

## 你会用它构建什么

Tenet 不是个人助手小玩具——它是一个通用原语，服务于**任何"必须当前为真、且历史可审计"的状态**。
下表每种模式都运行在本仓库已交付的功能面上：

| 你在构建 | Tenet 消除的失败 | 已交付的功能面 |
|---|---|---|
| Claude Desktop / IDE / 任何 MCP 客户端里的智能体 | 事实变化后记忆悄悄过期 | MCP 服务器（`learn`/`recall`/`time_travel`/`doubts`） |
| LangGraph / LlamaIndex / LangChain / Mem0 风格应用 | 框架记忆把新旧值*都*留在提示里 | `BaseStore` · `BaseMemoryBlock` · `TenetMemory` 适配器、兼容 Mem0 的 API |
| 客服 / CRM / 运维机器人（账户状态） | 用已被取代的套餐、地址、权限作答 | 键控取代 + 翻新结果（RAG 从 100 塌到 50%，Tenet 保持 100%） |
| 带共享状态的多智能体系统 | 并发写入者互相践踏事实 | Majalis 模式：取代机制作为写入仲裁规则 |
| 任何受审计/合规约束的场景 | "决策 X 时我们相信什么？"无法回答 | 溯源 + `recall(as_of=…)` + `tenet timeline`/`export` |

它的扩展方式像一个库，而非一项服务：在一台笔记本上，读取从 1k 到 100k 条事实**平坦地保持约
9–12 ms**（嵌入 + 闭式数学，单个 SQLite 文件）——"加上记忆"永远不会变成"运维一个数据库集群"。
...
这是一次运行内对比（绝对数字与 §1–2 行不可比，二者协议不同）。ReMe 端到端运行其官方发布的流水线
（`auto_memory` 会话笔记 → `update_index` → `bm25_search`，reme-ai 0.4.1.1，独立 venv）；我们修复了它的
四个运行时缺陷以保证公平运行（已在证据文件中记录），并补充运行了 ReMe 自带的作答智能体
（向量+BM25 ReAct 循环），得分 37.0%（p=0.55），统计上无差异 —— 差距并非协议假象。Tenet 在每一类问题上
都胜过 ReMe；tenet 对 RAG 为 +3pp（n=100 下不显著）。ReMe 低于同条件 RAG（p≈9×10⁻⁶）是 ReMe 自身的结果，
并非我们的调参：有损笔记蒸馏 + 纯关键词检索丢掉了普通分块检索保留的细节。</sup>
...
阅读论文：**[`paper/tenet.pdf`](paper/tenet.pdf)**（8 页），或 [Markdown 版本](paper/tenet.md)。
...
pip install -e ".[local]"                   # bge-small 嵌入器，CPU —— 安装后完全离线运行
python examples/00_zero_key_demo.py         # 取代 + 时间回溯 + doubts，零 LLM 调用
```
实测冷启动：pip 缓存已热时**从 clone 到输出 94 秒**（`[local]` 附加项会拉取
sentence-transformers/torch，首次安装需下载约 1 GB 的 wheel，冷网络请预留几分钟；
此后每次运行均离线且即时）。
...
`pip install "tenet-memory[llamaindex] @ git+https://github.com/Nas01010101/tenet.git"` —— 与官方事实列表块不同，事实变化时*取代*而非互相矛盾）·
...
### 🏆 标准化基准：MemoryAgentBench FactConsolidation（arXiv:2507.05257，全部 800 题）

冲突消解——知名记忆系统失败得最惨的轴（原表：单跳 **Zep 7%、Mem0 18%、MemGPT 28%**；
多跳全部 22 个系统 **≤7%**；长上下文推理基线达到 28）：
